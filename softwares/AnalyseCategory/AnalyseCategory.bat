...
:: Modifié pour scribe par David Pailler
...
cscript %SOFTWARE%\AnalyseCategory.js > NUL
...
reg.exe query "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\Shell Folders" /v "Common Programs" | %SystemRoot%\System32\find /I "Common Programs" > c:\tmp.txt
CHCP 1252 > NUL 
IF EXIST "C:\ProgramData" ( echo seven
	for /F "tokens=3* delims=	 " %%a in (c:\tmp.txt) do ( 
	CHCP 850 > NUL
...
SET raccprogstoclassperso=%SOFTWARE%\windows\PackagesCategory.txt
