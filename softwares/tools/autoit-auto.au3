...
; Comportement :
; 1. pour chaque prog.xml nécessaire, on descend un fichier %SOFTWARE%/softwares/prog/prog.ini contenant les noms des fenêtres et
;     boutons à cliquer, étape par étape (voir structure ci-dessous)
; 2. Dans le cas où une clé de licence est demandée, l'administrateur doit effectuer une copie de ce fichier en %SOFTWARE%/softwares/prog/prog-perso.ini
...
; 1. le xml de l'application doit appeler <install cmd='%SOFTWARE%\tools\autoit-auto.exe %SOFTWARE%\prog\prog.ini' />
; 2.  Dans ce dossier %SOFTWARE%\prog, il faut créer un fichier prog.ini qui contient les sections et fenêtres à fermer.
...
; 0. Si pas d'argument 2, le script teste si %SOFTWARE%\prog\prog-key.ini (le même que passé en argument 1 mais avec le suffixe "-key") existe
; 1. le nom du fichier ini perso qui contient la clé de licence : ne sera pas écrasé lors d'une maj wpkg du package.
; 2. le Nom du programme afin que l'autoit aille chercher la clé de licence dans le fichier "%SOFTWARE%\site\wpkg_apps.ini"
...
If $CmdLine[0] < 2 Then
	...
	If @error Then
		...
		$licence_ini = EnvGet("SOFTWARE") & "\site\wpkg_apps.ini"
		IniReadSectionNames($licence_ini)
		If @error Then
			If $debug > 1 Then
...
; on essaie de trouver le numero de licence dans "%SOFTWARE%\site\wpkg_apps.ini" sinon dans "%SOFTWARE%\prog\prog-key.ini"
; A IMPLEMENTER : il faut chercher la section quand on en a besoin dans action_programmee()
;$section = "test"
;IniReadSection($licence_ini, $section)
;If @error Then
	;$licence_ini = EnvGet("SOFTWARE") & "\" & $prog & "\" & $prog & "-key.ini"
	;IniReadSection($licence_ini, $section)
	;If @error Then
		;If $debug = "1" Then
...
Func _ActionLanceur()
	...
	; action run
	If $actionrun <> "" Then
		; problème avec l'antislash compris dans EnvGet("SOFTWARE") qui est interprété : il faut donc le doubler.
		$cheminZ=StringRegExpReplace(EnvGet("SOFTWARE") ,"\\", "\\\\" )
		$cheminProgramFiles=StringRegExpReplace(EnvGet("ProgramFiles") ,"\\", "\\\\" )
		$Commande=StringRegExpReplace(StringRegExpReplace($actionrun,"%SOFTWARE%", $cheminZ ),"%ProgramFiles%", $cheminProgramFiles )
		If $debug = "1" Then
