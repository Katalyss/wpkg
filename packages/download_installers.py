...
def get_vars(pkgxml):
    ...
    """
    if not pkgxml.has_key('variable'): return {}
    var_list = []
...
def gen_download(dl_list, pkgid):
    ...
    for dl_dict in dl_list:
        try:
            src = dl_dict['dl']
            if not dl_dict['destname']:
                msg = """
*** ERREUR *** Il manque la balise "destname=" dans "eoledl" pour %s
Exemple : destname="7zip/7z%%version%%.msi"
"""%src
                print(msg)
                continue
            ...
            msg = "Erreur : %s"%traceback.format_exc()
            print(msg)
            print e, src
    write_script('')
